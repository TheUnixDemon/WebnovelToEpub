...
class Argument:
    def __init__(self):
        ...
        parser.add_argument("-D", "--debughtml", action="store_true", help = "enable html view for debug output", default = False)
    ...
    def getDebugHtml(self) -> bool:
        return self.__args.debughtml if self.__args.debug else False
    
    def returnArguments(self):
        print(f"url:{self.getUrl()}, title:{self.getTitle()}, author:{self.getAuthor()}, cover:{self.getCover()}, filename:{self.getFilename()}, debugmode:{self.getDebug()}, debughtml:{self.getDebugHtml()}")